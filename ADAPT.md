...
>    «прочитай `ADAPT.md` и проведи меня по мастеру адаптации». Claude ведёт по фазам (профиль → хвост →
>    продукт), спрашивая блоками — см. секцию «Мастер адаптации — пошагово».
...
## Мастер адаптации — пошагово (Claude ведёт форкера)

> Эта секция — **сценарий для Claude**. Когда форкер говорит «проведи меня по адаптации» (или ты видишь
> незаполненный `repo-profile.md`), выполняй по фазам: спрашивай **блоками**, а не по одной метке; значения
> пиши сразу в файлы; не заставляй начинать «с нуля». Ориентир — ~30–60 минут вместо ручного обхода сотен меток.

### Фаза 1 — профиль, слои 1–2 (механическая подстановка)

Цель — заполнить `plugins/delivery/context/repo-profile.md`. Спрашивай **тремя блоками**, каждый — одним
сообщением (список полей), не по одному ключу:

- **Блок «Инфраструктура»:** git-хост команды · чат-канал для запросов доступа · дев-VPN-клиент (или «нет») ·
  scope PAT (push + API).
- **Блок «Стек репо»:** локальный путь к клону фронт-репо · группа/путь репо на хосте · путь к приложению
  (`apps/<app>`) · мажор Node · команда bootstrap-сертификатов · локальный dev-хост + порт · API-хост стейджа ·
  путь к e2e-сьюте · как проставить `data-test`.
- **Блок «i18n» (опц.):** есть ли i18n-платформа/TMS? Нет — удали i18n-строки профиля и i18n-шаги; да —
  env-переменная токена · CLI-команда.

Получил значения блока → впиши в колонку «Значение» профиля (замени `⟪ADAPT⟫`). Разумные дефолты предлагай
сам (напр. порт `8443`), но подтверждай. Это **детерминированная** часть — не импровизируй со значениями.

### Фаза 2 — инлайн-хвост one-off

`grep -rn 'ADAPT:' .` (кроме `ADAPT.md` и профиля) → оставшиеся уникальные метки. По каждой:
- **выводимо из профиля** (композиция путей: `⟦APP_PATH⟧/src`, `⟦FRONTEND_REPO⟧/⟦E2E_PATH⟧`, dev-URL
  `https://⟦DEV_HOST⟧:⟦DEV_PORT⟧`) — подставь сам, покажи, не переспрашивай;
- **`🔧`-блок** — следуй инструкции внутри (заполнить или удалить);
- **иначе** — спроси коротко, пачкой оставшихся (не блок за блоком).

### Фаза 3 — слой 3, продукт (СВОБОДА, не квиз)

Файлы `context/platform.md` · `context/dca.md` (центральный объект — переименуй под свой домен) ·
`context/analytics.md` · `delivery_qa_smoke/references/{platform-map,test-data,auth-scenario}.md`.

Здесь **не гони по вариантам**. Попроси форкера **описать своими словами** (2–4 предложения на файл): что за
продукт и ключевые поверхности; центральный объект домена и его жизненный цикл; как устроена аналитика; как
логиниться в тест-окружение. Дай **разумный дефолт-скелет** и разреши **отложить** (наполнить по ходу первых
задач). Свобода описания важнее полноты — это живое знание продукта, а не поля формы.

### Фаза 4 — именование + верификация

- Опц. переименовать плагин (`plugins/delivery/` → своё имя + `plugin.json`) и marketplace
  (`.claude-plugin/marketplace.json`).
- **Проверка «адаптация завершена»:** `grep -rn 'ADAPT:' .` → пусто (кроме этого `ADAPT.md`); профиль без
  `⟪ADAPT⟫`; каждый `⟦KEY⟧` имеет значение.
- Поставить marketplace + плагин (см. «Установка у себя» ниже) и прогнать `/deliver` — скиллы видны, значения
  подставляются.

**Принцип мастера:** жёстко скриптуется только механическая подстановка (фазы 1–2, значения в профиль/хвост);
где человек описывает продукт (фаза 3) — **свобода и дефолт**, а не гонка по пяти вариантам на каждый вопрос.
