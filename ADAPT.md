...
2. **Заполни профиль (главное):** `plugins/delivery/context/repo-profile.md` — единая таблица
   повторяющихся значений (git-хост, пути, версия Node, e2e, i18n); заполнил → подставятся во все `⟦KEY⟧`.
   Затем `grep -rn 'ADAPT:' .` — оставшиеся инлайн one-off метки (меньшинство) + `🔧`-блоки
...
**Сначала — профиль (главное).** Повторяющиеся значения (git-хост, путь к репо, e2e-путь, версия Node,
i18n-команда и т.п.) вынесены в **`plugins/delivery/context/repo-profile.md`** — заполни его **один раз**,
и они подставятся везде. Файлы механики ссылаются на них как `⟦KEY⟧` и при адаптации **не редактируются**
→ обновления апстрим-шаблона (`git merge`/`git pull`) ложатся в форк **чисто**. Потребитель — Claude:
читает профиль в контекст и подставляет значения. Остальное — инлайн-метки ниже.

Три вида мест, все грепаются:

| Маркер | Где | Что делать |
|---|---|---|
| `⟦KEY⟧` | ссылка на ключ профиля (git-хост, пути, версии, e2e, i18n…) | **не трогать инлайн** — заполнить значение в `context/repo-profile.md`. Пример: `cd ⟦FRONTEND_REPO⟧` |
| `⟪ADAPT: что вписать⟫` | inline one-off (уникальное, в одном месте) | вписать своё по месту |
| `> 🔧 **ADAPT — тема.** …` | блок с развёрнутым комментарием | заполнить/удалить по инструкции в блоке |
