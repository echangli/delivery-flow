...
- **Автотест-импакт — гейт стадии 5 (до push):** когда дифф трогает `⟪ADAPT: путь к приложению фронта, напр. apps/web-client⟫`, прогнать `autotest_impact` — задел ли дифф спеки e2e-сьюты (переименован/убран `data-test`, изменён лейбл/флоу/роут). 🔴-находки показать человеку **до** push. Тесты флоу **никогда не редактирует** — импакт уходит хэндоффом QA (product-side аккомодации — сохранить `data-test`/лейбл — можно предлагать). Прогон-подтверждение — при доступном слоте (стадии смоука).
