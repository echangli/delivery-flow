...
**Фолбэк** (папки задачи ещё нет — быстрый фикс / кросс-задачный прогон): плоские пути в рабочей папке
(`prototypes/YYYY-MM-DD-<задача>.html`, `delivery-notes-*.md`, `delivery-retro-*.md`); обросло
артефактами — свернуть в `tasks/<слаг>/`.
...
Вести **сырой лог по ходу** — все затыки, тупики, зависания, нестандартные решения. Дефолт — **`notes.md`
в папке задачи** (`tasks/<тикет>-<слаг>/notes.md`); фолбэк без папки — датированный
`delivery-notes-YYYY-MM-DD.md` в рабочей папке (см. § 9 / `workspace-layout.md`).

В конце флоу (или по запросу) собрать из него **ретро-синтез** — чистый файл с actionable-правками по
скиллам, сгруппированными и приоритизированными (🔴/🟡/🟢). Дефолт — **`retro.md` в папке задачи**;
фолбэк без папки — `delivery-retro-YYYY-MM-DD-<задача>.md` в рабочей папке.
