# Конвенции автотестов (`⟦E2E_PATH⟧`) — карта и правила

Источник правды по устройству боевого Playwright-сюита `<frontend>/⟦E2E_PATH⟧`. Используется скиллами
...
## Что где проверять — уровень теста (стоимость ↓, стабильность ↑)

До того как писать браузерный e2e — выбери уровень. Браузер дорогой и упирается в нестабильные
селекторы; не всё тащить туда. Бери **самый левый** уровень, что честно покрывает кейс:

1. **Чистая логика** (валидация, «пока N полей пустые — сабмит заблокирован», маппинг кода ошибки →
   сообщение, форматирование) — **юнит-тест** на юнит-слое репо (⟪ADAPT: команда юнит-тестов, напр. ci:test⟫),
   без браузера. Вынеси логику в чистую функцию/хук — десятки полей формы в браузер не тащатся.
2. **Детерминированная граничная ошибка** (413, конкретная форма 422, network-fail/offline) —
   **браузерный тест со стабом `page.route`**: перехватить ответ, вернуть фикс-статус/тело. Стабильно.
3. **Happy-path, загрузка файлов, всё что проверяет реальный контракт** — **живой стейдж, без мока**.
   Мок здесь узаконил бы неверный контракт, если дока разъехалась с бэком.

Итог: браузерный e2e держит happy-path + один негатив на границу; вариации формы и детерминированные
ошибки — на юнит/стаб-уровень. Побочно снимается часть боли с `data-test`.

> **Гейт по инфраструктуре — проверь, что репо тянет уровень, прежде чем предлагать.** `page.route` —
> стандарт Playwright (есть всегда). Юнит-слой — есть, если есть ⟪ADAPT: команда юнит-тестов⟫.
> **Компонентный рендер-тест** (напр. `@testing-library/react` в jsdom) — только если он заведён в репо;
> частый случай: jsdom + чистые юнит-тесты есть, а `@testing-library` НЕТ — тогда ограничься чистой
> логикой (уровень 1) + стабами, компонентные тесты не выдумывай. Нет уровня — честно скажи, используй доступный.
...
- **Нужен залогиненный/пополненный юзер:** `import { test } from '../fixtures/authed';` и в describe:
  ...
  заголовки из `api/gateway.ts`). API-хост — `E2E_API_URL` (дефолт `⟦STAGE_API_HOST⟧`).
...
- **Generic `data-test` (гоча).** Кнопки/инпуты из ⟪ADAPT: пакет вашей дизайн-системы⟫ часто рендерятся с
  ...
  через ⟦DATA_TEST_HELPER⟧ в компоненте фронта (отдельным коммитом в ту же ветку).
- **Не выдумывай id** — сверяй с живой поверхностью (скилл `selectors_sync`) или кодом фронта
  (`grep -r "data-test\|⟦DATA_TEST_HELPER⟧" ⟪ADAPT: путь к исходникам фронта, напр. apps/web-client/src⟫/<модуль>`).
...
- Против слота: `SLOT_URL` в `⟦E2E_PATH⟧/.env` (образец `example.env`; `⟪ADAPT: скрипт копирования env, напр. init:staging⟫` копирует) или в
...
- Проверки кода: `⟪ADAPT: команды проверки типов + линта, напр. npm run ci:type, npm run ci:eslint⟫` (из `⟦E2E_PATH⟧`).
